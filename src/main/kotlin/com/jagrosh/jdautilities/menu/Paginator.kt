...
class Paginator(
    ...
    fun display(channel: TextChannel): String {
        ...
        return paginate(channel, 1)
    }

    fun display(message: Message) = display(message.textChannel)

    fun paginate(channel: TextChannel, page: Int): String {
        if (list.isEmpty()) {
            return channel.sendMessage(renderEmptyPage()).submit().join().id
        }

        val pageNum = page.coerceIn(1, list.size)
        val msg = renderPage(page)
        return initialize(channel.sendMessage(msg), pageNum)
    }

    fun paginate(message: Message, page: Int): String {
        if (list.isEmpty()) {
            return message.editMessage(renderEmptyPage()).submit().join().id

        }

        val pageNum = page.coerceIn(1, list.size)
        val msg = renderPage(page)
        return initialize(message.editMessage(msg), pageNum)
    }
    ...
    private fun initialize(action: RestAction<Message>, page: Int): String {
        var id = ""
        action.submit()
            .thenCompose { m ->
